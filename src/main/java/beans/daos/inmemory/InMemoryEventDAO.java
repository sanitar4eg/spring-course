...
/**
 * Created with IntelliJ IDEA. User: Dmytro_Babichev Date: 2/3/2016 Time: 10:41 AM
 */
...
public class InMemoryEventDAO implements EventDAO {
	...
	public void delete(Event event) {
		...
		if (Objects.nonNull(events)) {
			final List<Event> toRemove = events.stream().filter(foundEvent -> foundEvent.getId() == event.getId())
				.collect(
					Collectors.toList());
	...
	public List<Event> getForDateRange(LocalDateTime from, LocalDateTime to) {
		return getEventStream().filter(event -> event.getDateTime().isAfter(from) && event.getDateTime().isBefore(to))
			.collect(
				Collectors.toList());
	}
